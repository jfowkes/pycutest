...
# TODO test reporting, nnzh/nnzj for fixed variables, cl/cu wrong for ALLINITC?

def array_compare(x, y, thresh=1e-8):
    return np.max(np.abs(x - y)) < thresh


def allinit_obj(x):  # ALLINTI* objective
    return np.array([
...
# TODO uncomment (makes other tests quicker)
...
#         self.assertIsNone(p.sifOptions , msg="Have sifOptions")
#         self.assertTrue(np.all(p.vartype == 0), msg="Not all variables are real")
#         self.assertEqual(p.nnzh, 10, msg="Wrong nnzh")  # nnz of dense upper triangular 4x4 Hessian # TODO not updated for fixed variables yet
#         self.assertFalse(p.eq_cons_first, msg="eq_cons_first is True")
#         self.assertFalse(p.linear_cons_first, msg="linear_cons_first is True")
#         self.assertFalse(p.nonlinear_vars_first, msg="Nonlinear variables listed first")
...
#         self.assertEqual(p.nnzj, 2, msg="Wrong nnzj")  # TODO not updated for fixed variables yet
#         self.assertTrue(array_compare(p.v0, np.array([0.0])), msg="Wrong v0")
#         # self.assertTrue(array_compare(p.cl, np.array([0.0])), msg="Wrong cl")  # TODO shouldn't be zero?
#         self.assertTrue(array_compare(p.cu, np.array([0.0])), msg="Wrong cu")
#         self.assertEqual(p.is_eq_cons, np.array([False], dtype=np.bool), msg="Wrong is_eq_cons")
